...
class InventoryDebugService {
  static Future<Map<String, dynamic>> runDiagnostic() async {
    ...
    try {
      ...
      result['local_item_count'] = 0; // Esto se podría implementar consultando SQLite
      
      // 7. Test CRUD básico
      result['crud_test_success'] = result['inventory_table_exists'] == true && 
  ...
  static Future<bool> createInventoryTableIfNotExists() async {
    if (!SupabaseService.isReady) {
      developer.log('Supabase no está inicializado', name: 'InventoryDebugService');
      return false;
    }
    ...
    try {
      ...
      developer.log('Error creando tabla de inventario: $e', name: 'InventoryDebugService');
      return false;
    }
  }
  ...
  static void printDebugInfo(Map<String, dynamic> status) {
    developer.log('=== ESTADO DEL INVENTARIO ===', name: 'InventoryDebugService');
    status.forEach((key, value) {
      developer.log('$key: $value', name: 'InventoryDebugService');
    });
    developer.log('============================', name: 'InventoryDebugService');
  }
}