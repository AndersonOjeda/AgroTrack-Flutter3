...
class DeepLinkService {
  ...
  static bool _isEmailConfirmationLink(Uri uri) {
    ...
    if ((uri.scheme == 'http' || uri.scheme == 'https') && uri.host == 'agrotrack.app') {
      if (uri.path.contains('email-confirmation') || uri.path.contains('/auth/confirm')) {
        return true;
      }
    }

    // Fallback: generic Supabase patterns
    return (uri.fragment.contains('access_token') && uri.fragment.contains('type=signup')) ||
           uri.queryParameters.containsKey('access_token') ||
           uri.path.contains('/auth/confirm');
  }

  /// Manejar confirmación de email
  static Future<void> _handleEmailConfirmation(BuildContext context, Uri uri) async {
    try {
      ...
      if (accessToken != null) {
        LoggerService.info('Email confirmado exitosamente - tokens encontrados');
        ...
        if (currentSession != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('¡Perfil confirmado exitosamente! Bienvenido a AgroTrack'),
          ...
          _showErrorDialog(context, 'Confirmación procesada, por favor inicia sesión');
        }
      } else {
